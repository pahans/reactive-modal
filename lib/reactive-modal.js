...
ReactiveModal.initDialog = function (info){
  ...
  if(!info || !info.template){
    ...
    for(var button in info.buttons){
      ...
      if (newButton.label !== undefined) {
          newButton.label = new ReactiveVar((info.buttons[button].label instanceof ReactiveVar) ? info.buttons[button].label.get() : info.buttons[button].label);
